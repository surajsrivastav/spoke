...
describe('webhook handler', () => {
  ...
  it('GET with missing mode returns 403 with failure text', async () => {
    const { app } = await import('../index.js');
    const res = await app.request(
      '/webhook/whatsapp?hub.verify_token=spoke_verify_token&hub.challenge=12345',
    );
    expect(res.status).toBe(403);
    const text = await res.text();
    expect(text).toBe('Verification failed');
  ...
  it('GET with missing challenge returns 403 with failure text', async () => {
    const { app } = await import('../index.js');
    const res = await app.request(
      '/webhook/whatsapp?hub.mode=subscribe&hub.verify_token=spoke_verify_token',
    );
    expect(res.status).toBe(403);
    const text = await res.text();
    expect(text).toBe('Verification failed');
  ...
  it('GET with wrong mode returns 403', async () => {
    const { app } = await import('../index.js');
    const res = await app.request(
      '/webhook/whatsapp?hub.mode=unsubscribe&hub.verify_token=spoke_verify_token&hub.challenge=12345',
    );
    expect(res.status).toBe(403);
    const text = await res.text();
    expect(text).toBe('Verification failed');
